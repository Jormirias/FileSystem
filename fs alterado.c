...
int fs_open(char *name, int openmode) {
    ...
    if (inode_load(ROOTINO, &root_inode) < 0) {
        return -1;
    }
    ...
    for (int i = 0; i < DIRBLOCK_PER_INODE && root_inode.dir_block[i] != 0; i++) {
        disk_read(root_inode.dir_block[i], block.data);
        
        
        for (int j = 0; j < DIRENTS_PER_BLOCK; j++) {
            if (block.dirent[j].d_ino != 0 && 
                strcmp(block.dirent[j].d_name, name) == 0) {
                found = 1;
                file_inode_num = block.dirent[j].d_ino;
                break;
            }
        }
        if (found) break;
    ...
    if (inode_load(file_inode_num, &file_inode) < 0) {
        return -1;
    }
 ...
 *  returns 0 on sucess; or -1 if fd is not a valid file descriptor
 */
int fs_close(int fd) {
    // TODO
    return -1;
